...
public class JwtGatewayFilter implements GlobalFilter, Ordered {
    ...
    public JwtGatewayFilter(@Value("${security.jwt.secret:dev-secret-change-me-dev-secret-change-me}") String secret) {
        ...
        log.info("JWT secret loaded (length={})", secret == null ? 0 : secret.length());
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            log.warn("JWT rejected for path={} method={} reason={}: {}", path, method, e.getClass().getSimpleName(), e.getMessage());
            exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);
            return exchange.getResponse().setComplete();
        }
